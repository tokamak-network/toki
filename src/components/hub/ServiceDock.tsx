...
// First-party hub services. Responsive: 2-col grid on mobile, vertical dock on desktop.
const APPS: App[] = [
...
export default function ServiceDock() {
  ...
  return (
    <section>
      <h2 className="text-sm font-semibold text-gray-300 mb-3 px-1">
        {t.hub.whatToDo}
      </h2>
        ...
        {APPS.map((app) => {
          ...
          return app.route ? (
            <Link
              key={app.id}
              href={app.route}
              className={`${base} bg-black/35 border-white/10 hover:border-accent-cyan/40 hover:bg-black/45 hover:scale-[1.02]`}
            >
              {inner}
            </Link>
          ) : (
            <div
              key={app.id}
              aria-disabled
              className={`${base} bg-black/25 border-white/10 opacity-60 cursor-not-allowed`}
            >
              {inner}
            </div>
          );
